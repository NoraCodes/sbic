...
fn read_config(filename: &Path) -> Configuration {
    ...
    let mut f = match File::open(filename) {
        Ok(f) => f,
        Err(e) => {efail(format!("Failed to open file {}: {}", filename.to_string_lossy(), e));}
    };
    ...
    let config: Configuration = match toml::from_str(&config) {
        Ok(c) => c,
        Err(e) => {efail(format!("Failed to parse configuration file: {}", e))}
    };
...
fn main() {
    ...
    if args.len() != 2 {
        efail("You must specify a configuration file!".into())
    }
